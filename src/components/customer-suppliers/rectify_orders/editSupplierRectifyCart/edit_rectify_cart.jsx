...
const EditRectifyCart = (props) => {
  ...
  const columns = [
    ...
    //   ),
    // },
    { field: "name", headerName: trans("Name"), width: 250 },
    {
      field: "unit",
    ...
    },
    {
      ...
      flex: 0.4,
      minWidth: 70,
      renderCell: (cellValues) => <p>{parseFloat(cellValues.value).toFixed(2) + " "}</p>,
  ...
  const DeleteProduct = async (row) => {
    ...
    // var check = RectifyOrderData[0].order_items.filter(
    //   (e) => e.product_id !== row.id
    // );
    // console.log("ccccc", check);
    // setRectifyOrderData((RectifyOrderData) => [
    //   RectifyOrderData,
    //   {order_items:check},
    // ]);
    // setRectifyOrderData;
    // ( [...RectifyOrderData, {order_items: check} ]);
    ...
    if (loc.state != undefined) {
      SweetAlert.fire({
        ...
        if (result.value) {
          axios
            .get(URL + `/removerectify/${loc?.state?.order_id}/${row.id}`, {
              headers: {
  ...
  const SubmitCartItems = async () => {
    if (ProductCart.length > 0) {
      ...
      if (localStorage.getItem("user_id") != null) {
        ...
        axios
          .post(
            URL + `/rectify/add/${SelectedCartData.supplier_name}`,
            formData,
            {
              headers: {
                Authorization: "Bearer " + localStorage.getItem("token123"),
          ...
          .then((response) => {
            if (response.data.success == true) {
              ...
              //       dispatch(SupplierCustomerCartTotal(response.data.cart_page_array.length));
              //     });
              // };
              // GetCartCount();
              toast.success(
              ...
              navigate("/supplierCustomerRectifyOrder/RD");
            }
          });
      } else {
        toast.error(trans("Please Login To Continue"));
...
export default translate(EditRectifyCart);
