...
const SupplierProducts = (props) => {
  ...
  const [supplierProList, setSupplierproducts] = useState([]);
  ...
  const bulkDeleteApiCall = async () => {
    await axios({
      ...
      const deleted = supplierProList.filter((item) => !BulkSelectionDeleteIds.includes(item.id))
  ...
  const columns = [
    ...
    {
      field: "category",
      headerName: trans("Category"),
      flex: 1,
      renderCell: (cellValues) => {
        return (
          <p>
            {cellValues.row.category != undefined &&
              cellValues.row.category.name}
          </p>
        );
      },
      valueGetter: (cellValues) =>
        cellValues.row.category != undefined && cellValues.row.category.name,
    },
    {
      field: "supplier",
      ...
      renderCell: (cellValues) => {
        return (
          <p>
            {" "}
            {cellValues.row.supplier != undefined &&
              cellValues.row.supplier.name}
          </p>
        );
      },
      valueGetter: (cellValues) =>
        cellValues.row.supplier != undefined && cellValues.row.supplier.name,
    },
    {
      field: "unit",
      ...
      renderCell: (cellValues) => {
        return (
          <div className="d-flex justify-content-center align-items-center w-100">
            {
              (role == "SuperAdmin" || (permissions.match("edit") != null)) && <Link className=""
                to={
  ...
  const downloadCSV = () => {
    axios({
      ...
      saveAs(blob, `suppliers_products.csv`);
    });
  };
      ...
      <Fragment>
        ...
        <Container fluid={true}>
          <Card>
              ...
              <Row>
                {(role == "SuperAdmin" || (permissions.match("create") != null)) && (
                  <Col className="text-right">
                    <Link to="/supplier/products/create/RD" className="mr-2">
            ...
            <CardBody>
              <DataGrid columns={columns} rows={supplierProList}
