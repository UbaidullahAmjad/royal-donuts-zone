...
const AllergeneTable = (props) => {
    ...
    await axios({
      ...
      if (response.data.success === true) {
        ...
        const getAllergenes = async () => {
          const response = await axios.get(`${URL}/allergen`, {
            headers: {
              Authorization: "Bearer " + localStorage.getItem("token123"),
            },
          });
          console.log("allergene", response);
          const products = response.data.allergens;
          products.map((item, index) => (item["index"] = index + 1));
          setAllergenes(products);
        };
        ...
        return (
          <div className="avatar ratio">
            <Media
              ...
              src={`${SIMPLE_URL}/images/Allergen/` + props.row.image}
              alt="#"
          ...
          <div>
            {
              (role == "SuperAdmin" || (permissions.match("edit") != null)) && <Link
                to={`/allergens/edit/${cellValues.row.id}/RD`}
              >
  ...
  const bulkDeleteApiCall = async () => {
    await axios({
      ...
      const deleted_allergen= allergenes.filter((item) => !BulkSelectionDeleteIds.includes(item.id))
        const index_update = deleted_allergen;
        index_update.map((item, index) => (item["index"] = index + 1));
        setAllergenes(index_update);
      SweetAlert.fire({
        icon: "success",
        title: trans("Deleted"),
  ...
  const DeleteItem = async (id) => {
    ...
    const res = await axios
      ...
      .then((response) => {
        ...
        const deleted_product = allergenes.filter((item) => item.id != id);
  ...
  return (
    <Fragment>
      <Breadcrumb parent={trans("Ecommerce")} title={trans("Allergens")} />
      <Container fluid={true}>
        <Card>
            ...
            <Row>
              {(role == "SuperAdmin" || (permissions.match("create") != null)) && (
                <Col className="text-right">
                  <Link to="/allergens/create/RD" className="mr-2">
          ...
          <CardBody>
            <DataTable columns={columns} rows={allergenes} 
