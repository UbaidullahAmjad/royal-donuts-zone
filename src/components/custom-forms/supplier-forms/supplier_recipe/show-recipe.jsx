...
const RecipeList = (props) => {
  ...
  const bulkDeleteApiCall = async () => {
    await axios({
      ...
      const deleted = RecipeAllListData.filter((item) => !BulkSelectionDeleteIds.includes(item.id))
        ...
        return (
          <div>
            <div style={{ marginLeft: 8, display: 'flex', flexWrap: 'wrap' }}>
              {
                cellValues.row.product_ingredients && cellValues.row.product_ingredients.map((pd_item, i) => (i <= 2 && <div className="mb-1" style={{ marginRight: 8, }}>
              ...
              {
                cellValues.row.extra_product_ingredients && cellValues.row.extra_product_ingredients
                && cellValues.row.extra_product_ingredients.map((ex_pd_item, i) => (i <= 1 && <div className="mb-1">
                  <Badge color="success" pill>{ex_pd_item.name}</Badge>
                </div>))
              }
            </div>
          </div>
      ...
      <Fragment>
        ...
        <Container fluid={true}>
          <Card>
                ...
                {(role == "SuperAdmin" ||
                  permissions.match("create") != null) && (
                    <Col className="text-right">
                      <Link to="/supplier/recipe/create/RD" className="mr-2">
            ...
            <CardBody>
              <DataGrid columns={columns} rows={RecipeAllListData}
